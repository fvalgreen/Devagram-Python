    ...
    def decodificar_token_jwt(token: str):  # Define uma função que irá desencriptografar o token
        try:
            token_decodificado = jwt.decode(token, JWT_SECRET,
                                            algorithms="HS256")  # Decodifica o token passado de acordo
            # com a chave
            ...
            if token_decodificado["expires"] >= time.time():  # Se o tempo de expiração for maior que o horário atual
                ...
                return None  # Se o tempo for menor ao horário atual retorna None indicando que o token é inválido

        except Exception:
            return None

    async def login_service(self,
                            usuario: UsuarioLoginModel):  # define a função login service recebendo como parametro um
        # usuário seguindo o modelo UsuarioLoginModel
        usuario_encontrado = await usuarioRepository.buscar_usuario_por_email(usuario.email)  # busca por email o
        # usupário
        ...
        if not usuario_encontrado:  # Caso não exista o email cadastrado volta um erro de email ou senha incorretos
            ...
            if authUtils.verificar_senha(usuario.senha, usuario_encontrado['senha']):  # Caso a senha informada e a
                # senha que
                # retornou do banco de dados sejam iguais
                usuario_encontrado[
                    'senha'] = ''  # Definimos a senha que retornou do banco de dados como uma string vazia
                # para não trafegar a senha do usuário
                token = self.gerar_token_jwt(
                    usuario_encontrado["id"])  # Geramos um token para o usuário passando o ID do msm
